...
<h1 class=" mt-5 font-bold text-2xl text-slate-700 text-center">Batches List</h1>
<div class=" mt-10 relative overflow-x-auto shadow-md sm:rounded-lg">
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            <?php
            $sql = "SELECT *,(SELECT COUNT(id) FROM enrollments WHERE batches.id = enrollments.batch_id) AS student_count FROM batches LEFT JOIN courses ON courses.id = batches.course_id";

            $query = mysqli_query($connect, $sql);
            echo "<pre>";
            while ($joinRows = mysqli_fetch_assoc($query)) :
                // print_r($joinRows);
            ?>
                <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                    ...
                    <td title="<?= $joinRows['title'] ?>" class="px-6 py-4">
                        <?= $joinRows['short'] ?>
                    </td>
                    <td class="px-6 py-4">
                        <?= date("d M y", strtotime($joinRows['start_date'])) ?>
                    </td>
                    <td class="px-6 py-4">
                        <?= date("gA", strtotime($joinRows['start_time'])) ?>-<?= date("gA", strtotime($joinRows['end_time'])) ?>
                    </td>
                    <td class="px-6 py-4">
                        <?= $joinRows['student_count'] ?>
                    </td>
                    <td class="px-6 py-4">
                        <?php if ($joinRows['is_register_open']) : ?>
